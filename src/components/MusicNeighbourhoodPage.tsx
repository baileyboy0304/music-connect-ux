...
export function MusicNeighbourhoodPage() {
  ...
  const loadArtist = async (artistName: string) => {
    ...
    try {
      ...
      const allAlbums = (search?.result?.album || search?.result?.albums || []).filter((a: any) => artistListIncludes(extractArtists(a), artistName));
      ...
      const allTracks = (search?.result?.track || search?.result?.tracks || []).filter((t: any) => artistListIncludes(extractArtists(t), artistName));
  ...
  useEffect(() => {
    ...
    try {
      ...
      const list = Array.isArray(playersData?.result) ? playersData.result : [];
      const mapped = list.map((p: any) => ({ id: p.player_id || p.id, name: p.display_name || p.name || 'Unknown', status: p.state || p.status || 'unknown', currentTrack: p.current_media?.name || p.current_item?.name || 'N/A', currentArtist: p.current_media?.artist || p.current_item?.artist || 'N/A', raw: p }));
      setPlayers(mapped);
